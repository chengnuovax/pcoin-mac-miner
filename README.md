# pcoin-intel-mac-miner

一键在 **Intel Mac（x86_64，macOS）** 上从源码构建并运行 [PCoin (PCN)](https://github.com/pars5555/pcoin) 的 headless 节点 + RandomX CPU 连池挖矿脚本。

PCoin 官方 release 只提供 Windows 二进制（`pcoin-win64-miner.zip`），不提供 macOS / Linux 可执行文件。本脚本在 Mac 上原生编译，无需改任何源码：RandomX 是 in-tree 子模块且以 x86_64 为主要目标，矿池客户端与 CPU miner 使用 `std::thread` + 跨平台 socket，无平台硬编码。

> 仅在 **Intel Mac** 上原生构建测试。Apple Silicon（M 系列）脚本也能跑（Homebrew 路径已兼容），但属于 Rosetta/原生自行验证范围，RandomX 在 ARM 上表现不同。
...
| `./pcoin-mac.sh mine [地址] [线程数]` | 连池开挖（线程数默认 = 物理核数） |
...
- 线程数 = **物理核数**，不要用逻辑核数。RandomX 为 ALU/内存带宽密集型，同一物理核上的第二个超线程只会抢资源，实测**加超线程反而降速且更热**。
- Intel MacBook 在持续满载下会降频：以 **30 分钟后的稳态算力**为准，不要看开头一分钟的爆发值。
- 关注机身温度；笔记本建议垫高散热、接电源。
...
$PCOIN_HOME/
├── src/                     # pcoin 源码 + build/
│   └── build/bin/           # bitcoind, bitcoin-cli
├── data/
...
- 编译产物为 ad-hoc 签名，现代 Mac 上可通过 Gatekeeper；无 hardened runtime 但 JIT（RandomX 需要）正常工作。
